...
district_lqas_performance <- function(data,
  ...
  label_districts <- isTRUE(show_district_labels)
  ...
  # before (country_key/district_key already computed above).
  if (is.null(ambiguous_district_pairs)) ambiguous_district_pairs <- character(0)
  district_layer$.country_key  <- norm_text(district_layer$ADM0_NAME)
  district_layer$.district_key <- norm_text(district_layer$ADM2_NAME)
  district_layer$.province_key <- if ("ADM1_NAME" %in% names(district_layer)) norm_text(district_layer$ADM1_NAME) else NA_character_
  district_layer$.pair <- paste(district_layer$.country_key, district_layer$.district_key, sep = "||")
  district_layer$.match_key <- ifelse(
    district_layer$.pair %in% ambiguous_district_pairs,
    paste(district_layer$.pair, district_layer$.province_key, sep = "||"),
    district_layer$.pair
  )

  processed_data$.pair <- paste(processed_data$country_key, processed_data$district_key, sep = "||")
  processed_data$.match_key <- ifelse(
    processed_data$.pair %in% ambiguous_district_pairs,
    paste(processed_data$.pair, processed_data$province_key, sep = "||"),
    processed_data$.pair
  )

  processed_data <- apply_singleton_fallback(district_layer, processed_data, ambiguous_district_pairs)
  ...
  # once per month inside the loop below. left_join() with an sf x-side
  # re-attaches (and effectively duplicates) the ENTIRE district_layer's
  # geometry on every call -- looping it per month meant re-duplicating
  # every district's polygon (matched or not, since left_join keeps every
  # district_layer row) once per month, only to immediately discard the
  # unmatched (NA performance) rows right after. A single join across the
  # whole processed_data (all months at once) followed by split()-ing the
  # result by month produces the exact same per-month row set, just
  # without the repeated full-geometry duplication.
  joined_all <- suppressWarnings(
    dplyr::left_join(
      district_layer,
      processed_data,
      by = ".match_key",
  ...
  for (sel_date in unique_months) {
    ...
    lqas_performance <- joined_by_month[[as.character(sel_date)]]
    if (is.null(lqas_performance) || nrow(lqas_performance) == 0) next
    ...
    if (isTRUE(label_districts)) {
      label_data <- lqas_performance |>
        sf::st_as_sf() |>
        dplyr::group_by(ADM0_NAME, ADM1_NAME, ADM2_NAME) |>
        dplyr::slice(1) |>
        dplyr::ungroup()
    ...
    district_counts <- lqas_performance |>
      sf::st_drop_geometry() |>
      dplyr::distinct(ADM0_NAME, ADM1_NAME, ADM2_NAME, performance) |>
