...
is_local_dev <- interactive() && Sys.getenv("RSTUDIO") == "1"
...
preprocess_shapefile <- function(data) {
  if (is.null(data)) return(NULL)
  if (inherits(data, "sf")) {
    data %>% select(any_of(c("ADM0_NAME", "ADM1_NAME", "ADM2_NAME", "geometry")))
  } else data
}
...
sample_size_main <- if (SAMPLE_DATA) 10000 else NULL
sample_size_scope <- if (SAMPLE_DATA) 5000 else NULL
sample_size_admin <- if (SAMPLE_DATA) 5000 else NULL

cat("📊 Sampling strategy:", if(SAMPLE_DATA) "ACTIVE" else "INACTIVE", "\n")
...
load_shapefiles_safely <- function() {
  ...
  shapes <- list(
    countries = tryCatch({
      sf_data <- load_data_simple("africa_countries.rds", required = FALSE)
      if (!is.null(sf_data)) {
        preprocess_shapefile(sf_data)
      } else {
        cat("⚠️ Country shapes not available\n")
        NULL
      }
    }, error = function(e) {
    ...
    provinces = tryCatch({
      sf_data <- load_data_simple("africa_provinces.rds", required = FALSE)
      if (!is.null(sf_data)) {
        preprocess_shapefile(sf_data)
      } else {
        cat("⚠️ Province shapes not available\n")
        NULL
      }
    }, error = function(e) {
    ...
    districts = tryCatch({
      sf_data <- load_data_simple("africa_districts.rds", required = FALSE)
      if (!is.null(sf_data)) {
        preprocess_shapefile(sf_data)
      } else {
        cat("⚠️ District shapes not available\n")
        NULL
      }
    }, error = function(e) {
...
print_data_summary <- function() {
  ...
  cat("🌍 Environment:", if(is_deployment) "DEPLOYMENT" else "LOCAL DEVELOPMENT", "\n")
  cat("📊 Sampling:", if(SAMPLE_DATA) "ACTIVE" else "INACTIVE", "\n")
...
load_data_safely <- function() {
  tryCatch({
    dat <- readRDS("input/AFRO_LQAS_data_c.rds")
    cat("Successfully loaded data with", nrow(dat), "rows\n")
    return(dat)
  }, error = function(e) {
...
dat <- load_data_safely()

# Load scope data
scope <- NULL
tryCatch({
  scope <- readRDS("input/Scope.rds")
  scope <- scope %>%
    mutate(
      round_start_date = as.Date(round_start_date),
      ...
      vaccine.type = ifelse(is.na(vaccine.type), "nOPV2", vaccine.type)
    )
  cat("Loaded Scope data with", nrow(scope), "rows\n")
}, error = function(e) {
  warning("Scope.rds not found: ", e$message)
})

# Load shapefiles
load_shapefile <- function(file_name) {
  paths <- c(
    file.path("input", file_name),
    file_name,
...
all_countries <- load_shapefile("africa_countries.rds")
all_provinces <- load_shapefile("africa_provinces.rds")
all_districts <- load_shapefile("africa_districts.rds")
        ...
        conditionalPanel(
          ...
          column(
            ...
            fluidRow(
              ...
              valueBoxOutput("scope_countries", width = 3),
              valueBoxOutput("scope_districts", width = 3),
          ...
          column(
            ...
            box(
              ...
              infoBox(
                ...
                value = HTML("
          <small>
            <strong>Pie/Donut:</strong> Best for composition (≤8 units)<br>
            <strong>Heatmap:</strong> Best for 8-20 units<br>
            <strong>Bar charts:</strong> Best for >20 units<br>
            <strong>Treemap/Sunburst:</strong> Hierarchical proportions<br>
            <strong>Radar:</strong> Multi-dimensional comparison<br>
            <strong>Lollipop:</strong> Elegant alternative to bars<br>
            <strong>Waterfall:</strong> Shows cumulative effect
          </small>
        "),
                subtitle = NULL,
        ...
        conditionalPanel(
          ...
          column(
            ...
            fluidRow(
              ...
              valueBoxOutput("unresolved_countries", width = 4),
              valueBoxOutput("unresolved_districts", width = 4)
            ),
...
server <- function(input, output, session) {
  ...
  output$scope_countries <- renderValueBox({
    req(scope_analysis())
    result <- scope_analysis()
  ...
  output$scope_districts <- renderValueBox({
    req(scope_analysis())
    result <- scope_analysis()
    n <- if (!is.null(result$data_summary$overall_summary) && nrow(result$data_summary$overall_summary) > 0) {
  ...
  observeEvent(input$unresolved_analyze, {
    ...
    safe_analysis({
      ...
      result <- generate_unresolved_NC_plots_DS_optimized(
        data = dat,
        x = x,
        y = y,
  ...
  output$unresolved_districts <- renderValueBox({
    req(unresolved_analysis(), input$unresolved_reason)
  ...
  output$unresolved_countries <- renderValueBox({
    req(unresolved_analysis(), input$unresolved_reason)
